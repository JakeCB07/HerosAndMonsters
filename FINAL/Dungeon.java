...
public class Dungeon {
	...
	private static char[][] getSurroundingArea(Hero hero) {
		...
		//above
		if(heroY >0)
			area[1][0] = room[heroX][heroY-1].getRoomSymbol();
		//upper right
		if(heroX < 4 && heroY > 0)
			area[2][0] = room[heroX+1][heroY-1].getRoomSymbol();
		//middle left
		if(heroX > 0)
			area[0][1] = room[heroX-1][heroY].getRoomSymbol();
		//middle
		area[1][1] = room[heroX][heroY].getRoomSymbol();
		//middle right
		if(heroX < 4)
			area[2][1] = room[heroX+1][heroY].getRoomSymbol();
		//bottom left
		if(heroX > 0 && heroY < 4)
			area[0][2] = room[heroX-1][heroY+1].getRoomSymbol();
		//bottom middle
		if(heroY < 4)
			area[1][2] = room[heroX][heroY+1].getRoomSymbol();
		//bottom right
		if(heroX < 4 && heroY < 4)
			area[2][2] += room[heroX+1][heroY+1].getRoomSymbol();
